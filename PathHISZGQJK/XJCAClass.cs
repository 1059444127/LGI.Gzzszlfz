...
{ 
    ...
    class XJCAClass
    {
        ...
        [DllImport("XJCA_HOS.dll", EntryPoint = "XJCA_SslEmptyCache", CharSet = CharSet.Auto, SetLastError = false)]
        public static extern void XJCA_SslEmptyCache();
        ...
        public string CaQZ(string blh, string bglx, string bgxh, string debug, DataTable dt_cg, string yhmc,string bgzt)
        {
            ...
            if (bglx == "bc" || bglx == "bd")
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                try
                                {
                                    string sqlstr = "insert into T_SZQM(F_blbh,F_blh,F_RQ,F_bgys,F_bgyw,F_TimeSignXML,F_signxml,F_cert,F_certID) values ('" + blbh + "','" + blh + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + yhmc + "','" + bgyw + "','" + TimeSign_XML + "','" + signxml.ToString() + "','" + cert + "','" + certID + "') ";

                                    int x = aa.ExecuteSQL(sqlstr);
                    ...
                    string upbmpfs = f.ReadString("CA", "upbmpfs", "1").Replace("\0", "").Trim();
