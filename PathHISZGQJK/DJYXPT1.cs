...
namespace PathHISJK
{
    public class DJYXPT
    {
        ...
        public void Create_PDF(string blh, string bglx, string bgxh, string msg, string debug, string[] cslb)
        {
            string qxsh = "";
            string xdj = "";
            bglx = bglx.ToLower();
            ...
            if (bgzt == "已审核" || bgzt == "取消审核")
            {
                ...
                if (bgzt2.Trim() == "已审核" && bgzt != "取消审核")
                {
                    ...
                    if (f.ReadString("savetohis", "ispdf", "1").Replace("\0", "").Trim() == "1")
                    {
                        ...
                        bool isrtn = CreatePDF(blh, bglx, bgxh, ref message, ref jpgname);
                        ...
                        if (isrtn)
                        {
                            ...
                            {
                                bool ssa = UpPDF(blh, jpgname, ML, ref message, int.Parse(xy));
                                if (ssa == true)
                                {
                                    if (debug == "1")
...
//        //CA归档
//        public void CAGD(string pdfname, string blh, string bglx, string bgxh,DataTable Dt_jcxx )
//            {
...
//            if(bglx=="bc")
//            {
//                Dt_bcbg = aa.GetDataTable("select * from T_BCBG where F_blh='" + blh + "' and F_BC_BGXH='" + bgxh + "' and F_BC_BGZT='已审核' ", "bcbg");
//            }

//            if (bglx == "bd")
//            {
//                Dt_bdbg = aa.GetDataTable("select * from T_BdBG where F_blh='" + blh + "' and F_Bd_BGXH='" + bgxh + "' and F_Bd_BGZT='已审核' ", "bdbg");
//            }
...
//xmlstring=xmlstring+"<medicaldoc>";
//xmlstring=xmlstring+"<hospitalcode>医院代码</hospitalcode>";
//xmlstring = xmlstring + "<systemcode>0503</systemcode>";
//xmlstring = xmlstring + "<patientcode>" + Dt_jcxx.Rows[0]["F_BRBH"].ToString() + "</patientcode>";
//xmlstring=xmlstring+"<admission>住院次数</admission>";
//xmlstring=xmlstring+"<index>文档序号</index>";
//xmlstring=xmlstring+"<total>文档总数</total>";
//xmlstring=xmlstring+"<docuniquedesc>"+"BL_"+blh+"_"+bglx+"-"+bgxh+"</docuniquedesc>";
//xmlstring=xmlstring+"<doctitle>病理报告</doctitle>";
//xmlstring=xmlstring+"<docdesc>文档描述</docdesc>";
//xmlstring=xmlstring+"<docfileformat>归档文件MIME类型</docfileformat>";
//xmlstring=xmlstring+"<docfileverifydata>归档文件验证数据</docfileverifydata>";
//xmlstring=xmlstring+"<docfileverifytype>归档文件验证方式</docfileverifytype>";
//xmlstring=xmlstring+"<verifydatas>";
//xmlstring=xmlstring+"<verifydata>";
//xmlstring=xmlstring+"<plaindata>原文</plaindata>";
//xmlstring=xmlstring+"<signdata>原文签名值</signdata>";
//xmlstring=xmlstring+"<signtimestamp>时间戳</timestamp>";
//xmlstring=xmlstring+"<cert>证书</cert>";
//xmlstring=xmlstring+"</verifydata>";
//xmlstring=xmlstring+"<verifydata>";
//xmlstring=xmlstring+"<plaindata>原文</plaindata>";
//xmlstring=xmlstring+"<signdata>原文签名值</signdata>";
//xmlstring=xmlstring+"<signtimestamp>时间戳</timestamp>";
//xmlstring=xmlstring+"<cert>签名证书</cert>";
//xmlstring=xmlstring+"</verifydata>";
//xmlstring=xmlstring+"</verifydatas>";
//xmlstring=xmlstring+"</medicaldoc>";
...
//                string caurl = f.ReadString("bjxwyy", "caurl", "http://ip:port/medicaldocs/").Replace("\0", "");
//                HttpWebRequest request = WebRequest.Create(caurl) as HttpWebRequest;
...
//                    request.AddHeader("authorization", "Basic 安全密钥");
//                    request.AddHeader("accept", "application/xml");
//                    request.AddHeader("accept-Language", "zh-CN,zh;q=0.8,en-US;q=0.5,en;q=0.3");
//                   // request.AddHeader("content-type", "multipart/form-data; boundary=分隔符");
//                    request.ContentType = "multipart/form-data; boundary=" + boundary;
//                    byte[] multipartPostData = BuildMultipartPostData(boundary, pdfname, xmlstring);
...
//                    sr.Close();
//                    response.Close();
//                    string inxml = responseData;
//                    MessageBox.Show(inxml);

//                    StringReader xmlstr2 = null;
//                    XmlTextReader xmread = null;
//                    xmlstr2 = new StringReader(inxml);
//                    xmread = new XmlTextReader(xmlstr2);
...
//            }
        ...
        public static byte[] BuildMultipartPostData(string Boundary, string pdfname, string xmlstr)
            {
                StringBuilder sb = new StringBuilder();
                sb.AppendLine("--" + Boundary);
                sb.AppendLine("Content-Disposition: form-data;name=\"medicaldocxml\"; filename=\"medicaldocxml.xml\"");
                sb.AppendLine("Content-Type: application/octet-stream");
                sb.AppendLine("Content-Transfer-Encoding: binary");
                sb.Append(Environment.NewLine);
                sb.AppendLine(xmlstr);

                sb.AppendLine("--" + Boundary);
                sb.AppendLine("Content-Disposition: form-data;name=\"medicaldocfile\"; filename=\"Report.pdf\"");
                sb.AppendLine("Content-Type: application/octet-stream");
                sb.AppendLine("Content-Transfer-Encoding: binary");
                sb.Append(Environment.NewLine);

                byte[] pdffile = System.IO.File.ReadAllBytes(pdfname);
        ...
        public bool CreatePDF(string F_blh, string bglx, string bgxh, ref string err_msg, ref string jpgname)
        {
            ...
            try
            {
                ...
                if (bgzt == "已审核")
                {
                    ...
                    string rptpath = f.ReadString("ZGQJK", "rptpath", "rpt").Replace("\0", "").Trim();
                    jpgname = "";
