    ...
    public class DJYXPT
    {
        ...
        public void djyxpt_hx(string F_blh, string bglx, string bgxh, string bgxj, string sfsave, string yymc)
        {
            ...
            else
            {
                ...
                if (bljc.Rows[0]["jsbz"].ToString() == "0")
                {
                    try
                    {
                    ...
                    try
                    {
                        conn.Open();
                        OCMD.ExecuteNonQuery();
                        aa.ExecuteSQL("update t_jcxx set jsbz='1' where f_blh='" + F_blh + "'");
                    }
                    catch (Exception ex)
                    {
                        if (debug == "1")
            ...
            #region AStatusGate接口

            string bgzt = "";
            switch (bljc.Rows[0]["f_bgzt"].ToString())
            {
                case "已写报告":
                    bgzt = "BL01";
                    break;
            ...
            try
            {
                ...
                OCMD.CommandText = "select SGATE_STATUS_INFO_sequence.nextval from dual";
                conn.Open();
                OracleDataAdapter oda = new OracleDataAdapter(OCMD);
                ...
                sgateid = Convert.ToInt32(dt1.Rows[0][0].ToString());
            ...
            string sub_type = "ADD";
            if (bljc.Rows[0]["F_bgzt"].ToString() != bljc.Rows[0]["jlzt"].ToString())
            {



                string sqlstring1 = "insert into SGATE_STATUS_INFO (PK,PATIENT_ID,PATIENT_DOMAIN_ID,PAT_NAME,PAT_CATEGORY,PAT_CATEGORY_SYSTEM,DOCUMENT_NAME,DOCUMENT_UNIQUE_ID,DOCUMENT_DOMAIN_ID,PAY_LOAD_TYPE,SUB_TYPE,REQUEST_NUMBER,REQUEST_DOMAIN,ORDER_NUMBER,ORDER_DOMAIN,EVENT_STATUS,EVENT_TIME,EVENT_OPERATOR_ID,EVENT_OPERATOR_NAME,EVENT_HIUP_STATUS,EVENT_HIUP_ERROR_INFO,EVENT_CREATE_TIME,EVENT_START_TIME,EVENT_END_TIME,EVENT_RETRY,EVENT_RETRY_TIME,DCM_PATIENT_ID,DCM_STUDY_IUID,DCM_ACCESSION_NO,DCM_MODALITY_AE,DCM_EXAM_METHOD,DCM_BODY_PART,DCM_IMG_NUMBER,DCM_STATUS,DICOM_STUDY_TIME) VALUES(" +
                          "'" + sgateid + "'," +
                          "'" + F_blh + "'," +
                          "'" + pisid + "'," +
                          "'" + bljc.Rows[0]["F_xm"].ToString() + "'," +
                          "'" + brlb + "'," +
                ...
                OCMD.CommandText = sqlstring1;
                OCMD.Parameters.Clear();
                log.WriteMyLog(sqlstring1);
                try
                {
                    conn.Open();
                    ...
                    if (result > 0)
                    {
                        aa.ExecuteSQL("update t_jcxx set jlzt='" + bljc.Rows[0]["F_bgzt"].ToString() + "' where f_blh='" + F_blh + "'");
                    }
                }
                catch (Exception ex)
                {
                ...
                try
                {
                    ...
                    OCMD.CommandText = "select SGATE_EXTEND_ID_INFO_sequence.nextval from dual";
                    conn.Open();
                    OracleDataAdapter oda = new OracleDataAdapter(OCMD);
                    ...
                    sgateexid = Convert.ToInt32(dt1.Rows[0][0].ToString());
                ...
                sqlstring1 = "insert into SGATE_EXTEND_ID_INFO (PK,STATUS_FK,ID,DOMAIN_ID) VALUES(" +
                          "'" + sgateexid + "'," +
                          "'" + sgateid + "'," +
                          "'" + brbh + "'," +
                          "'2.16.840.1.113883.4.487.5.6.10')";
